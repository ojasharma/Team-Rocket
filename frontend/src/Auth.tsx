    ...
    <div className="flex items-center justify-center min-h-screen bg-black text-white nunito bg-gradient-to-br from-black to-gray-900 px-4 py-6">
      <div className="w-full max-w-sm bg-gray-900 rounded-lg shadow-lg border border-gray-800 transition-all duration-300">
        <div className="text-center p-4">
          ...
          <h2 className="text-lg font-semibold text-gray-300">{isSignUp ? "Create Account" : "Welcome Back"}</h2>
          <p className="text-gray-500 text-xs mt-1">
            {isSignUp ? "Join our community of founders and investors" : "Sign in to access your account"}
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="px-4 pb-4 space-y-4">
          ...
          <div className="mt-6">
            <button 
              ...
              className={`w-full p-2.5 bg-yellow-500 text-black font-bold rounded-md hover:bg-yellow-600 transition-all transform active:scale-95 text-sm ${isSubmitting ? 'opacity-75 cursor-wait' : 'cursor-pointer'}`}
            >
              {isSubmitting ? (
                <span className="flex items-center justify-center">
                  ...
                  <span className="text-xs">{isSignUp ? "Creating Account..." : "Signing In..."}</span>
                </span>
              ) : (
        ...
        {!isSignUp && (
          <div className="px-4 pb-2 text-center">
            <a href="#" className="text-yellow-500 text-xs hover:underline">Forgot your password?</a>
          </div>
        )}
